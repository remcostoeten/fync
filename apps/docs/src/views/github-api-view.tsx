...
export function GithubApiView() {
  const tocItems = useToc({ sections: githubContent })
      ...
      <div className="flex-1 space-y-12">
        <div className="space-y-4">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl">
            Comprehensive GitHub API client with authentication and resource management. 
            Build powerful integrations with GitHub's REST API using type-safe, chainable methods.
          </p>
...
const repos = await github.user('octocat').repos.get()`}
        ...
        <div className="space-y-16">
          {githubContent.map(function renderSection(section, sectionIndex) {
            return (
              <section
                key={section.id}
                id={section.id}
