...
export function pickFields<T>(items: T[], picker: TFieldPicker<T>): Partial<T>[] {
...
export function paginateItems<T>(items: T[], limit?: number, offset?: number): T[] {
...
export function applyFilterOptions<T>(items: T[], options: TFilterOptions<T>): T[] {
  let result = [...items];
