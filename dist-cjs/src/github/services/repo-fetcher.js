...
async function fetchRepositoriesInternal(username, opts = {}) {
  ...
  const response = await (0, _httpClient.httpRequestWithPagination)(`https://api.github.com/users/${username}/repos`, defaultParams, signal);
...
const memoizedFetchRepositories = (0, _cacheService.memoize)(fetchRepositoriesWrapper, (...args) => {
  const username = args[0];
  const opts = args[1] || {};
  const {
    signal: _signal,
