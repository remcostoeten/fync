...
function filterRepositoriesByForks(repos, min, max) {
    return repos.filter((repo) => {
        if (min !== undefined && repo.forks_count < min)
            return false;
        if (max !== undefined && repo.forks_count > max)
            return false;
        return true;
    });
}
function filterRepositoriesByStars(repos, min, max) {
    return repos.filter((repo) => {
        if (min !== undefined && repo.stargazers_count < min)
            return false;
        if (max !== undefined && repo.stargazers_count > max)
            return false;
        return true;
    });
}
function filterRepositoriesByWatchers(repos, min, max) {
    return repos.filter((repo) => {
        if (min !== undefined && repo.watchers_count < min)
            return false;
        if (max !== undefined && repo.watchers_count > max)
            return false;
        return true;
    });
}
...
function filterRepositoriesByAdvancedCriteria(repos, criteria) {
    ...
    if (criteria.minWatchers !== undefined ||
...
export { filterRepositories, filterRepositoriesByAdvancedCriteria, filterRepositoriesByLanguage, filterRepositoriesByForks, filterRepositoriesByStars, filterRepositoriesByWatchers, filterRepositoriesByTopics, filterRepositoriesByLicense, };
//# sourceMappingURL=repository-filters.js.map