...
async function fetchRepositoriesInternal(username, opts = {}) {
    ...
    const response = await httpRequestWithPagination(`https://api.github.com/users/${username}/repos`, defaultParams, signal);
...
const memoizedFetchRepositories = memoize(fetchRepositoriesWrapper, (...args) => {
    const username = args[0];
    const opts = args[1] || {};
