...
type THttpClient = {
  get<T>(path: string, params?: Record<string, any>): Promise<{ data: T }>;
  ...
  getPaginated?<T>(path: string, params?: Record<string, any>): Promise<{ data: T[]; nextUrl?: string }>;
};
  ...
  async function executeRequest<T = unknown>(
    ...
    async function requestFn(): Promise<T> {
      switch (method) {
        ...
        case "POST":
          return (await httpClient.post(path, data)).data;

        case "PUT":
          return (await httpClient.put(path, data)).data;

        case "PATCH":
          return (await httpClient.patch(path, data)).data;
    ...
    get(target, prop: string | symbol) {
      ...
      if (prop === "paginate") {
        function paginateHandler<T = unknown>(options?: TRequestOptions) {
          return executeRequest<T[]>("GET", undefined, { ...options, paginate: true });
        }
