...
const GITHUB_OAUTH2_PROVIDER: TOAuth2Provider = {
	name: "github",
...
function validateGitHubScopes(scopes: readonly string[]): void {
	const validScopes = Object.values(GITHUB_OAUTH2_SCOPES);

	for (const scope of scopes) {
		if (!validScopes.includes(scope as TGitHubOAuth2Scope)) {
