		...
		const requestFn = async () => {
			...
			switch (method) {
				case "GET":
					...
					return httpClient.get(url, { headers: defaultHeaders });
				case "POST":
					return httpClient.post(url, data, { headers: defaultHeaders });
				case "PUT":
					return httpClient.put(url, data, { headers: defaultHeaders });
				case "PATCH":
					return httpClient.put(url, data, { headers: defaultHeaders });
				case "DELETE":
					return httpClient.delete(url, data, { headers: defaultHeaders });
				default:
	...
	return new Proxy(
		{},
		{
			get(target, prop) {
				...
				if (prop === "paginate") {
					return (options) =>
						executeRequest("GET", undefined, { ...options, paginate: true });
				}
				if (prop === "stream") {
